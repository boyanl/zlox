...
fn number() void {
    const val = std.fmt.parseFloat(vals.Value, parser.previous.lexeme) catch {
...
const rules = init: {
    ...
    var arr = [_]ParseRule{.{ .prefix = undefined, .infix = undefined, .precedence = undefined }} ** count;
    const t = s.Token.Type;
    const none = .{ .prefix = null, .infix = null, .precedence = .NONE };
